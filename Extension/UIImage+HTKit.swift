...
extension UIImage {
    ...
    func attributedString(_ origin: CGPoint, size: CGSize? = nil) -> NSAttributedString {
        let obj = NSTextAttachment()
        obj.image = self
        obj.bounds = .init(origin: origin, size: size ?? self.size)
        return NSAttributedString(attachment: obj)
    }
    
    func mutableAttributedString(_ origin: CGPoint, size: CGSize? = nil) -> NSMutableAttributedString {
        let obj = NSTextAttachment()
        obj.image = self
        obj.bounds = .init(origin: origin, size: size ?? self.size)
        return NSMutableAttributedString(attachment: obj)
    }
}
